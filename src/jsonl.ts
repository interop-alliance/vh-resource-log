 ...
 * as an error instead of a silently shorter history. A parse failure is the
 * fabrication class ({@link ResourceLogIntegrityError}): a body that does not
 * parse as the profile's format is a doctored or truncated log.
 *
 ...
 * authorization) belong to `verify.ts`, which runs over the parsed entries.
 */
 ...
 * its genesis entry) -- a caller-misuse guard, so a plain `Error` rather than
 * a refusal class: nothing served is being judged here.
 *
